...
namespace CommonMark.Parser
{
    internal static class TableMethods
    {
        static List<string> ParseTableLine(StringPart part, StringBuilder sb)
        {
            ...
            //TODO: Rewrite using line.IndexOfAny(new [] { '\', '|' } )
            for (; i < line.Length; i++)
            {
                var c = line[i];
                if (c == '\\')
        ...
        private static void MakeCell(string text, Block row, ref int offset)
        {
            ...
            // TODO: Dont create "trimmedText" var, calculate white spaces within text
        ...
        static void MakeTableRows(Block table, StringBuilder sb)
        {
            ...
            for (var i = 0; i < parts.Array.Length; i++)
            {
                ...
                // skip the header row
                if (i != 1 && !string.IsNullOrEmpty(line.Source) && line.Source != " ")
                {
                    ...
                    row.StringContent.Append(line.Source, 0, line.Length);
